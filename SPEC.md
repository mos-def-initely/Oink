...
### 9.1 Avatar pig
"Vinyl Toy" finish: glossy gradient shading, a specular highlight, **no outlines**, standing **upright on two legs**. Customisable colour/hat/accessory/background.

Anatomy rule: tail, legs, arms, torso, then head — each limb is overlapped by the piece drawn after it, so every joint is covered. The tail sits on the **lower back**, clear of the arms.

Fattening adds **belly rolls**, not just width:

| Places logged | Tier | Build |
|---|---|---|
| 0–4 | Slim | narrow torso, no rolls |
| 5–14 | Regular | wider, one fold |
| 15–29 | Chubby | two folds, double chin |
| 30+ | Round | three folds, arms pushed out |

Thresholds live only in `lib/pig.ts`.

**Face variant** is used on map pins and in dense lists; the **full body** appears only on the feed and the profile.
...
Loud colour on clean structure. The rule that keeps it from tipping into cartoon: **no black outlines, no hard offset shadows** — separation comes from colour and soft elevation.

- Ground: warm apricot / off-white. Loud accents: coral-red `#FF4D6D`, tangerine `#FF8A00`. Support: teal `#00B39F`. Ink: deep plum `#2B1B3D`.
- Type: Nunito (display, 700–900) + Nunito Sans (body).
- Generous rounding (16px cards), soft ambient shadow, buttons that scale slightly on press.
- Reaction icons are pigs: happy for Oink, angry for Shame. Never a clock or a bell.
